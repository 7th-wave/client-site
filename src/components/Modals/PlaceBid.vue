        ...
        >
          ...
          > 
            ...
            <div v-if="!showConversor">
              <div class="p-4 sm:items-start mt-3 text-left sm:mt-0">
                <label
                  class="
                    block
                    text-sm
                  ...
                  "
                >
                  Minimum bid: {{ minValue }} <span v-if="currency=='ETH'">(usd)</span> <span v-if="currency=='USDC'">USDC</span>
                </label>
                <div
                  class="
                    mt-2
                ...
                >
                    <div class="mt-1 flex rounded-md shadow-sm">
                      <span
                        ...
                        "
                      >
                        <USDC color="#000000" size="5" v-if="currency=='USDC'" />
                        <ETH color="#000000" size="5" v-if="currency=='ETH'" />
                      </span>
                      <input
                        type="number"
                        name="usd"
              ...
              <div class="px-4 pt-3 pb-3 flex flex-wrap justify-end border-t">
                <Button @on:submit="placeBid" v-if="currency=='ETH'" customClass="w-full" name="PLACE BID" />
                <Button @on:submit="placeBid" v-if="authorized && hasFunds && currency=='USDC'" customClass="w-full" name="PLACE BID" />
                <Button @on:submit="setAllowance" v-if="!authorized && hasFunds && currency == 'USDC'" name="PRE-AUTHORIZE BID" />
                <Button @on:submit="conversor" v-if="!authorized && !hasFunds && currentProvider == 'metamask'" name="CONVERT TO USDC" />
                <Button @on:submit="showFortmaticDeposit" v-if="!authorized && !hasFunds && currentProvider == 'fortmatic'" name="GET USDC" />
...
export default {
  ...
  emits: ["on:bid", "on:allowance", 'on:close'],
  props: ["open_modal", "value", "step", "auctionRef", "clientRef", 'currency'],
  setup(props, {emit}) {
    let { open_modal, value, step, auctionRef, clientRef } = toRefs(props);
    ...
    watch(value, (val) => {
      ...
      console.log('hasFunds->', hasFunds.value);
    });
    ...
    const setAllowance = async () => {
      ...
      if (bidTotal.value >= minValue.value) {
        try {
          const ERCReceipt = await tokenAllowance(Math.pow(10, 6), currentAddress.value, instances[currentProvider.value], currentProvider.value);
          console.log(ERCReceipt);

          store.dispatch('user/setAllowance', {client: clientRef.value.dbRef});
    ...
    const placeBid = async () => {
      if (bidTotal.value >= minValue.value) {
        ...
        try {
          ...
          const receipt = await bid(auctionRef.value, bidTotal.value, currentAddress.value, instances[currentProvider.value], currentProvider.value);
          console.log('receipt')
          console.log(receipt)
